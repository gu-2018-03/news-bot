...
async def process(db, channel, entries):
    ...
    # создание экземпляров парсера  HTML и "выделиля" ключевых слов
    # ----------------
    parser = kw.MyHTMLParser()
    kw_finder = kw.KeyWords2()
    # ---------------------
    ...
    for entry in entries:
        ...
        if published > last_published:
            ...
            data = await kw.fetch_async(news['link'])
            #---- минимальный парсинг  html
            parser.feed(data.decode('utf-8'))
            #---- Выделение ключевых слов
            kw_finder.set_text(parser.text_extracted)
            kw_finder.build_model(5)
            kw_finder.calculate_links()
            tags = kw_finder.get_keywords(10)
            #---- добавление ключевых слов к словарю новости
            news['tags'] = tags
